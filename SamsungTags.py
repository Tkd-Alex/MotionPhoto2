...
class SamsungTags:
    ...
    def __init__(self, video_bytes: bytes, image_type: str = "heic"):
        ...
        if self.image_type not in ["heic"]:
            ...
            self.tags["MotionPhoto_Data"] = bytes(
                "mpv2___.___.", "utf-8"
            )  # dummy data for length computation
    ...
    def get_image_padding(self) -> int:
        ...
        size = 0
        for tag, value in const.SAMSUNG_TAG_IDS.items():
            if tag in self.tags:
                size += len(value)
                size += 4  # place to put size of string on next line
                size += len(tag)
                if tag == "MotionPhoto_Data":
                    return size
            ...
            if tag in self.tags:
                ...
                for preceding_tag in const.SAMSUNG_TAG_IDS:
                    if preceding_tag in self.tags:
                        tag_offsets[preceding_tag] = tag_length + (
                            tag_offsets[preceding_tag]
                            if preceding_tag in tag_offsets
                            else 0
                        )
